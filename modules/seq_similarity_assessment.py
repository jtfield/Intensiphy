...
def multi_thread_seq_compare(working_dir):
    """Runs comparison program to assess and record sequence similarities"""
    ...
    for taxon_1 in list_of_taxa:
        ...
        if os.path.isdir(taxon_1_dir_path):
            ...
            multi_thread_sim_compare(df, _path1, _path2, _taxon_1, _seq_list, suffix)
            #################################################################


def multi_thread_prepare_compare(_df, _path1, _path2, _taxon_1, _seq_list, suffix):
    for taxon_2 in os.listdir(_seqs_dir):

        taxon_2_dir_path = _seqs_dir + '/' + _taxon_2

        if os.path.isdir(taxon_2_dir_path):

            taxon_2_seq_file = taxon_2_dir_path + '/' + _taxon_2 + _suffix
